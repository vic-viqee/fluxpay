...
class IdempotencyMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        user = await self.get_current_user(request) # Assuming a way to get user from request
        ...
        if request.method in ["POST", "PUT", "DELETE"] and response.status_code in [200, 201, 204]:
            ...
            if idempotency_key:
                ...
                try:
                    ...
                    response_body = {"message": "Non-JSON response saved"} # Handle non-JSON responses if necessary
    ...
    async def get_current_user(self, request: Request) -> User:
        ...
        try:
            ...
            raise HTTPException(status_code=401, detail=f"Could not get current user: {e}")
